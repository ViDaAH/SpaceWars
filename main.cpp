...
int main()
{
    ...
    // DECLARACIÓN DE VARIABLES PARA IMÁGENES DE CONTADOR INICIAL, NIVEL 1
    ...
    // DECLARACIÓN DE VARIABLES PARA IMÁGENES DE CONTADOR INICIAL, NIVEL 2
    ...
    // DECLARACIÓN DE VARIABLES PARA IMÁGENES DE CONTADOR INICIAL, NIVEL 3
    ...
    // DECLARACIÓN DE VARIABLES PARA IMÁGENES DE CONTADOR GENERAL, PAUSA-REANUDACIÓN
...
void menu()
{
    ...
    {
        al_get_mouse_state(&mouse_state); // OBTENCIÓN DEL PUNTO EN QUE SE ENCUENTRA EL CURSOR EN PANTALLA
        x = mouse_state.x;                // POSICIÓN DEL CURSOR RESPECTO AL EJE X
        y = mouse_state.y;                // POSICIÓN DEL CURSOR RESPECTO AL EJE Y

        // SI EL CURSOR ESTÁ SOBRE EL BOTÓN "SALIR", MOSTRAR IMAGEN CON "SALIR" RESALTADO
        ...
        // SI NO ESTÁ EL CURSOR SOBRE LOS BOTONES, MOSTRAR LA IMAGEN ESTÁNDAR
        if (!(x >= salir_x && x <= salir_x + botons_w && y >= salir_y && y <= salir_y + botons_h) &&
...
void contadores()
{
    // SELECCIÓN SEGUN INSTRUCCIÓN PRIMARIA (NIVEL:1/2/3) //SECUNDARIA <<0>> (PAUSA-REANUDACIÓN)
    switch (nivel)
    {
    case 1:
        cont_en = 40;
...
private:
    ...
    double posx = 1920 / 2 - 77, posy = 1000;                                       // Variable para manejar la Posición del Jugador en pantalla (variará según el momento e instrucciones, la posición defininda inicialmente es para cuando el juego inicial)
    double posx_min = 0, posx_max = 1843;                                           // Límite (Muro invisible) de movimiento del jugador <<en este caso, en el eje X>>
    double posy_min = 1000, posy_max = 700;                                         // Límite (Muro invisible) de movimiento del jugador <<en este caso, en el eje Y>>
    ...
    int cantdm = 0;                                                        // Cantidad de disparos máximos (se definirá según el nivel en que se esté
    ...
    int ancho_x = 75, alto_y = 75;                                           // Tamaño en x, y. De las naves enemigas
    int cont_enemigos = 0, vida_e[5], m_e;                                   // Vida de las 5 naves que se deberán representar en cada "ronda"
    double mov_ex[5], mov_ey[5];                                             // Movimiento inicial de las naves enemigas mientra "inic" está activa y ascx/ascy/dscx/dscy están desactivadas
    bool ascx[5], dscx[5], ascy[5], dscy[5], inic;                           // Movimiento de las naves dependiendo de la posición de las naves enemigos respecto a los bordes
    ALLEGRO_BITMAP* disparo_e = al_load_bitmap("recursos/juego/shot_e.png"); // Imagen de Disparo para naves enemigas

public:
    // Manejo y reestablecimiento de las variables para el juego (dentro de la ejecución de este)
    void preparar()
    {
        game_over = false;
        win = false;
    ...
    // Proceso para Finalización del Nivel en base a variables "game_over" o "win"
    void g_over()
    {
        ...
        if (game_over == true)
        {
            ...
            nivel = 0; // Se establece Nivel en 0 debido a que en contadores no se vuelve a leer el archivo por lo que, al evaluar en que nivel se está, requiere una re definición. En este caso, nivel=0;
            contadores();
        }
        ...
        if (win == true)
        {
            // Variable para reescribir el archivo según el punto y progreso actual
            ofstream archivo_e_n;

            // evaluando según nivel
            switch (nivel)
            {
            case 1:
                ...
                archivo_e_n << "2"
                    << " "
                    << vida << endl;
                nivel = 2; // Misma lógica que cuando se pierde, se establece el nivel en 0; en este caso: Nivel=2;
                break;
            ...
            case 3:
                ...
                // Establecer el nivel en 1 (ya que no hay más) e igual con vida=500
                archivo_e_n.open("recursos/informacion.txt", ios::out);
                archivo_e_n << "1"
                    << " " << 500 << endl;
            ...
            contadores(); // Regresar a contadores
        }
    }
    ...
    void mostrar()
    {
        ...
        if (win == false && game_over == false)
        {
            ...
            al_get_keyboard_state(&keyboard_state); // Obtención de l estado del teclado (que tecla se está presionando)
            player();
            enemigo();
        ...
        // Si se ha ganado o perdido, Llamar a "g_over()" donde se ejecutará según el caso, las instrucciones necesarias
...
void juego()
{
    // INICIALIZACIÓN DE INSTANCIA PARA LA CLASE "game"
    game j1;
    ...
    // Variable para manejo de ejecución del juego (para salir mediante ESC)
    bool salir = false;

    // Mientras salir=false (no se ha presionado "ESC" y nivel no sea "0" (no se perdido el juego/nivel)
    while (salir == false && nivel >= 1)
    {
        // Llamar a "mostrar()" que visualizará todo en pantalla (enemigos y sus disparos, fondo, jugador y sus disparos, contadores de vida, enemigos, disparos)
        j1.mostrar();

        // Si se presiona "ESC" del teclado, declarar "salir=true;" para que el bucle finalice y retorne a menu()". ESTO PUEDE SER TOMADO COMO UNA PAUSA, AUNQUE AL HACERLO, TENDRÁS QUE VOLVER A JUGAR EL NIVEL ENTERO. NO SE VERÁN AFECTADOS LOS DISPAROS DISPONIBLES NI LA VIDA (QUE YA SE TENÍA AL INICIAR EL NIVEL)
